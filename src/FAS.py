...
from .utils_constants import (
    axis_to_system_units,
    system_default_units,
    axis_units,
    Axis,
    system_to_axis_units,
)
...
class FiveAxisSystem:
    ...
    def _move_single_axis(self, axis: str, pos: int) -> str:
        ...
        return self.controller.send(
            f"{axis_command}-Achse:{int(pos)}", str(self.speed), str(self.acceleration)
        )
    ...
    def _parse_answer(self, answer) -> Position:
        ...
        if len(answer_without_ending) > 1:
            ...
            axis_values = [
                parse_number_string(split_axis[i].split(" ")[0]) for i in range(1, 6)
            ]
